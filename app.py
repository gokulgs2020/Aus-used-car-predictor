...
def add_bg_with_opacity(image_file, opacity=0.3):
    ...
    st.markdown(
        ...
        .stApp .main {{
            position: relative;
            z-index: 1;  /* ensures widgets are on top */
        }}
        </style>
        """,
...
# Add background image from repo (must be in the same folder as app.py)
...
input_data = pd.DataFrame([{
    ...
    "transmission_auto": int(transmission == "Automatic"),
...
with st.expander("ℹ️ Top 5 Features influencing price"):
...
with st.expander("ℹ️ Model Info"):
    ...
    - **Data**: Scraped from Australian car listing sites  
